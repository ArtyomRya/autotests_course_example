...
def everything_for_your_cat(cats_data):
    ...
    for i in range(len(cats_data)):
        shopper = ' '.join(cats_data[i][2:])
        cats_age = ', '.join(map(str, cats_data[i][:2]))
        dict[shopper] = f'{dict[shopper]}; {cats_age}' if dict.get(shopper) else cats_age

    our_str = ''
    for i in range(len(dict)):
        our_str = f"{': '.join(dict.popitem())}\n{our_str}"
